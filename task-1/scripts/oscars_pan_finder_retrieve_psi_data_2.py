...
# # OSCARS PaN-Finder
# ## Retrieve all publicly accessible and published data from data provider: PSI
# 
# All data is directly retrieved from PSI SciCat instance
...
data_provider_url = "https://dacat.psi.ch/api/v3/"
data_provider_url
...
while keep_going:
    ...
    #print(f"Skip : {skip}. Params: {params}. Status Code: {res.status_code}. Url: {res.request.url}. Number of results: {len(current_batch)}. Continue: {keep_going}. Total number of documents retrieved: {len(raw_documents)}")
    print(".",end="")
...
data_file_name = "../data/psi_open_data_" + datetime.datetime.now().strftime("%Y%m%d%H%M%S%f") + ".json"
data_file_name
